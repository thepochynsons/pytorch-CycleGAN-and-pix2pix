...
class CycleGANModel(BaseModel):
    ...
    def modify_commandline_options(parser, is_train=True):
        ...
        if is_train:
            ...
            parser.add_argument('--use_ssim', type=str, default=False, help='include ssim in the cycle loss')
        return parser
    ...
    def __init__(self, opt):
        ...
        if self.isTrain:
            ...
            self.fake_simeco_pool = ImagePool(opt.pool_size)  # create image buffer to store previously generated images
            self.fake_us_pool = ImagePool(opt.pool_size)  # create image buffer to store previously generated images
    ...
    def forward(self):
        ...
        self.fake_us = self.netG_simeco(self.real_simeco)  # G_A(A)
        self.rec_simeco = self.netG_us(self.fake_us)   # G_B(G_A(A))
        self.fake_simeco = self.netG_us(self.real_us)  # G_B(B)
        self.rec_us = self.netG_simeco(self.fake_simeco)   # G_A(G_B(B))
    ...
    def optimize_parameters(self):
        ...
        # G_A and G_B
        self.set_requires_grad([self.netD_simeco, self.netD_us], False)  # Ds require no gradients when optimizing Gs
